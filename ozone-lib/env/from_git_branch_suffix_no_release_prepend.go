...
func DynamicFromGitBranchSuffix(ordinal int, varsMap, fromIncludeMap *VariableMap) error {
	dirPath := "./"
	...
	if len(branchNameParts) != 2 || utils.ValidateName(branchName) == false {
	...
	namespace := fmt.Sprintf("%s", releaseVersionRFC1123)
